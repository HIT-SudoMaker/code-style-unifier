...
pub fn project_human(terminal: &ReviewTerminal) -> String {
    match terminal {
        ReviewTerminal::Rejected(rejection) => format!(
            ...
            rejection.code().escape_debug(),
            rejection.message().escape_debug()
        ),
        ReviewTerminal::Failed(failure) => format!(
            "Terminal: Failed\nDisposition: Failed\nError: {}: {}\n",
            failure.code().escape_debug(),
            failure.message().escape_debug()
        ),
        ReviewTerminal::Sealed(review) => {
            ...
            if review.findings().is_empty() {
                ...
                for finding in presentation_ordered_findings(review) {
                    ...
                    let _ = writeln!(
                        ...
                        finding.column(),
                        finding.rule(),
                        finding.subject().escape_debug(),
                    );
                    let _ = writeln!(
                        output,
                        "  Observation: {}",
                        finding.observation().escape_debug()
                    );
                    let _ = writeln!(
                        output,
                        "  Message: {}",
                        finding.message().escape_debug()
                    );
                    if let Some(question) = finding.question() {
                        let _ = writeln!(
                            output,
                            "  Question: {}",
                            question.escape_debug()
                        );
                    }
                }
            }
            if blocked.is_empty() {
                ...
                for detail in blocked {
                    let _ = writeln!(
                        output,
                        "- {} {:?}: {}",
                        detail.file.escape_debug(),
                        detail.family,
                        detail.reason.escape_debug(),
                    );
                }
            }
            output
