            ...
            AuthorityInput::Directory(directory) => {
                ...
                fs::read(&path).map_err(|error| {
                    let message = if error.kind() == std::io::ErrorKind::NotFound {
                        format!(
                            concat!(
                                "Authority not found at {}. ",
                                "Project orientation / Authority preparation: ",
                                "formal source review has not started. ",
                                "Confirm the selected path; the default project ",
                                "location is .csu/authority/authority.json. ",
                                "Inspect the project read-only and prepare ",
                                "evidence-backed candidate facts and owner ",
                                "questions before creating Authority. ",
                                "Missing facts are not source violations; ",
                                "absence does not prove a first-ever review. ",
                                "Original error: {}",
                            ),
                            path.display(),
                            error,
...
pub(crate) enum DependencyProfileLaw {
    Python {
        ...
        mixed_class_blocked: &'static str,
...
struct DependencyAuthorityInput {
    ...
    /// Python 原始分组内的固定排序检查是否适用
    #[serde(default)]
    python_reorder_safe: bool,
    /// Rust 原始分组内的固定排序检查是否适用
    #[serde(default)]
...
fn compile_dependency_law(
    ...
    parser
        ...
        .map_err(|error| {
            ReviewRejection::new(
                ...
                format!(
                    "cannot initialize Python dependency grammar: {error}"
                ),
            )
        })?;
    ...
    {
        if !python_dependency_root_is_valid(&mut parser, name)
            || !seen.insert(name)
        {
            return Err(ReviewRejection::new(
                "authority.dependency",
...
/// 用固定 Python 语法确认来源为一个原样标识符，不接纳路径或附加语句
fn python_dependency_root_is_valid(
    ...
    let Some(path) = statement.child_by_field_name("name") else {
        return false;
    };
    !root.has_error()
        ...
        && path.kind() == "dotted_name"
        && path.named_child_count() == 1
        && path.named_child(0).is_some_and(|identifier| {
            identifier.kind() == "identifier"
                && identifier.start_byte() == prefix.len()
                && identifier.end_byte() == prefix.len() + name.len()
        })
}
    ...
    rust_trait_path_is_valid(path)
        && generic
            ...
            .is_some_and(|lifetime| {
                // 未声明 edition 时保留旧版合法名称及生命周期专用名称
                ascii_identifier_is_valid(lifetime)
                    && (matches!(
                        lifetime,
                        "static" | "union" | "async" | "await" | "dyn" | "try"
...
const STANDARD_LAW: StandardLaw = StandardLaw {
    ...
    profiles: [
        ProfileLaw {
            identity: "python",
            observation_method: "tree-sitter-python@0.25.0+direct-source-facts-v2",
            ...
            dependency: DependencyProfileLaw::Python {
                ...
                mixed_class_blocked: "Python mixed-class import statement layout is unsupported; Authority classification cannot enable this capability",
        ...
        ProfileLaw {
            identity: "rust",
            observation_method: "tree-sitter-rust@0.24.2+direct-source-facts-v2",
            documentation: DocumentationLaw {
        ...
        ProfileLaw {
            identity: "c",
            observation_method: "tree-sitter-c@0.24.2+direct-source-facts-v2",
            documentation: DocumentationLaw {
            ...
            dependency: DependencyProfileLaw::Procedural {
                unavailable_blocked: "C/C++ dependency target and preprocessing analysis is unsupported; the current Authority schema has no input for this capability",
            },
        ...
        ProfileLaw {
            identity: "cpp",
            observation_method: "tree-sitter-cpp@8b5b49eb+direct-source-facts-v2",
            documentation: DocumentationLaw {
                carrier: DocumentationCarrierLaw::NativeAdjacent,
                summary: SummaryLocation::FirstNonempty,
                headings: &NATIVE_HEADINGS,
    ...
    rules: [
        ...
        RuleLaw {
            ...
            grade: FindingGrade::HardViolation,
            message: "callable documentation must satisfy applicable controlled roles in profile order",
            question: None,
            semantic_revision: 7,
            chapter: "Explain",
            presentation_contract: "callable_documentation",
