    ...
    fn collect_node(&mut self, node: Node<'_>, nested: bool) {
        ...
        match self.language {
            ...
            Language::Rust => match node.kind() {
                ...
                "use_as_clause" | "extern_crate_declaration" => {
                    let alias = node.child_by_field_name("alias");
                    if let Some(alias) = alias.filter(|alias| {
                        alias.utf8_text(self.source).ok() == Some("_")
                    }) {
                        self.push_rust_discard_declaration(alias);
                    } else {
                        self.push_named_declaration(
                            alias,
...
fn observe_callable(
    ...
    if language == Language::Rust
        && node.kind() == "ordered_field_declaration_list"
    {
        observe_rust_tuple_fields(node, source, output);
        return nested;
    }
...
/// 按类型字段的原始位置观察 Rust 公开元组字段，不创建作者名称
fn observe_rust_tuple_fields(
    node: Node<'_>,
    source: &[u8],
    output: &mut Vec<Callable>,
) {
    ...
    for index in 0..node.child_count() {
        ...
        if child.kind() == "visibility_modifier" {
            public = child.utf8_text(source).ok() == Some("pub");
        }
        if node.field_name_for_child(index) != Some("type") {
            continue;
        }
        if public {
            let attribute = observe_rust_attribute(child, source);
            let point = child.start_position();
            output.push(Callable {
                language: Language::Rust,
                name: format!("<tuple-field:{field_index}>"),
                line: point.row + 1,
                column: point.column + 1,
                named: false,
                ...
                template_parameters_complete: true,
                requires_template_parameters: false,
                return_shape: ReturnShape::Unknown,
                carrier: documentation_carrier(
                    Language::Rust,
                    child,
                    source,
                    Some(&attribute),
                ...
                carrier_unresolved: attribute.nonliteral_documentation,
            });
        }
        field_index += 1;
        public = false;
...
fn rust_public_documentation_item(
    node: Node<'_>,
    source: &[u8],
    attribute: Option<&RustAttributeFacts>,
) -> bool {
    match node.kind() {
...
fn observe_rust_attribute(
    ...
    while let Some(attribute) = preceding {
        let tuple_visibility = attribute.kind() == "visibility_modifier"
            && node.parent().is_some_and(|parent| {
                parent.kind() == "ordered_field_declaration_list"
            });
        let comment_trivia = attribute.utf8_text(source).is_ok_and(|text| {
            matches!(attribute.kind(), "line_comment" | "block_comment")
                && !text.starts_with("//!")
                && !text.starts_with("/*!")
        });
    ...
    for item in items {
        if matches!(item.kind(), "line_comment" | "block_comment") {
            if let Ok(text) = item.utf8_text(source)
                && (item.kind() == "line_comment"
                    && text.starts_with("///")
                    && !text.starts_with("////")
                    || item.kind() == "block_comment"
                        && rust_outer_block_documentation_opener_is_exact(
                            text,
                        ))
            {
                facts.documentation.extend(
                    rust_documentation_lines(text).iter().map(|line| {
                        format!("/// {}{}", line.left_padding, line.text)
                    }),
                );
            }
            continue;
        ...
        match path.utf8_text(source).ok() {
            ...
            Some("doc") => {
                let literal = attribute
                    ...
                    .and_then(|value| decode_rust_text_literal(value, source));
...
/// 解码 Rust 原生文本字面量，不执行宏或其他属性表达式
fn decode_rust_text_literal(node: Node<'_>, source: &[u8]) -> Option<String> {
    let text = node.utf8_text(source).ok()?;
    if node.kind() == "raw_string_literal" && text.starts_with('r') {
        return direct_child_of_kind(node, "string_content")
...
fn observe_documentation_visibility(
    ...
    nested: bool,
    name: &str,
    rust_attribute: Option<&RustAttributeFacts>,
        ...
        Language::Rust => {
            if (!nested
                ...
                || (matches!(
                    ...
                    .and_then(|body| body.parent())
                    .is_some_and(|owner| {
                        owner.kind() == "trait_item"
                            && rust_has_unrestricted_public_visibility(
                                owner, source,
                            )
                    }))
...
fn native_family_callable_is_proven_internal(
    ...
    let mut cursor = node.walk();
    node.named_children(&mut cursor).any(|specifier| {
        specifier.kind() == "storage_class_specifier"
            && specifier
                .utf8_text(source)
...
struct DependencyDeclaration {
    ...
    wildcard: bool,
    python_modules: Vec<PythonDependencyModule>,
    rust_tree: Option<RustUseTree>,
    module_placement_invalid: bool,
}

/// 保存 Python 导入语句中每个来源模块及其原始位置
#[derive(Clone, Debug)]
struct PythonDependencyModule {
    key: String,
    line: usize,
    column: usize,
    future_directive: bool,
}

impl PythonDependencyModule {
    /// future 指令来源由语法确定，普通导入仍需要项目分类事实
    fn tier(&self, authority: &CompiledAuthority) -> Option<u8> {
        if self.future_directive {
            Some(0)
        } else {
            authority.python_dependency_tier(&self.key)
        }
    }
}

/// 保存 Rust 导入路径、别名、部分子列表和原始位置
#[derive(Clone, Debug)]
struct RustUseTree {
    path: Vec<String>,
    tail: RustUseTail,
...
/// 区分普通绑定、通配和保留未知项的列表导入尾部
#[derive(Clone, Debug)]
enum RustUseTail {
    Name(String),
    Wildcard,
    List(Vec<Option<RustUseTree>>),
}

impl RustUseTree {
    /// 完整树才参与整体比较，未知子项不丢弃已知相邻事实
    fn is_complete(&self) -> bool {
        match &self.tail {
            RustUseTail::List(members) => members
                .iter()
                .all(|member| member.as_ref().is_some_and(Self::is_complete)),
            _ => true,
        }
    }
}
...
fn observe_dependencies(
    ...
    DependencyFacts { declarations }
}

/// 按语言允许的作用域收集连续导入，分组身份由同一遍历拥有
fn collect_dependency_groups(
    ...
    for child in scope.named_children(&mut cursor) {
        if language == Language::Python
            && child.kind() == "future_import_statement"
        {
            current_group = None;
            let group = *next_group;
            *next_group += 1;
            push_dependency_declaration(
                language, child, source, group, output,
            );
            continue;
        }
        ...
        if is_import {
            let group = *current_group.get_or_insert_with(|| {
                let group = *next_group;
                *next_group += 1;
                group
            });
        ...
        current_group = None;
        collect_dependency_groups(language, child, source, next_group, output);
    }
}
...
/// 保存所有可达预处理路径共同证明的位置事实
#[derive(Clone, Copy, Default)]
struct CplusplusModulePosition {
    ordinary_declaration_seen: bool,
    placement_invalid: bool,
}

impl CplusplusModulePosition {
    /// 按源码顺序观察一个声明，条件分支在出口合并
    fn observe(
        &mut self,
        node: Node<'_>,
        source: &[u8],
        ...
        match cplusplus_module_node(node) {
            CplusplusModuleNode::GlobalFragment => {
                self.placement_invalid = true;
            }
            CplusplusModuleNode::ModuleDeclaration => {
                self.placement_invalid = self.ordinary_declaration_seen;
            }
            CplusplusModuleNode::Import(import) => {
                push_cplusplus_module_import(
                    import,
                    source,
                    self.placement_invalid,
                    output,
                );
            }
            ...
            {
                *self = self.observe_branch(node, source, output);
            }
            CplusplusModuleNode::Other
                if node.kind() == "comment"
                    || node.kind().starts_with("preproc_") => {}
            CplusplusModuleNode::Other => {
                collect_nested_cplusplus_imports(node, source, output);
                self.ordinary_declaration_seen = true;
                self.placement_invalid = true;
            }
        }
    }

    /// 每个替代分支从相同入口开始，只保留全部出口都证明的事实
    fn observe_branch(
        self,
        node: Node<'_>,
        source: &[u8],
        ...
        for child in node.named_children(&mut cursor) {
            ...
            {
                branch.observe(child, source, output);
            }
        }
        if node.kind() == "preproc_else" {
            return branch;
        }
        // 没有 else 时，条件不成立的路径保持入口事实。
        let other = alternative.map_or(self, |alternative| {
            self.observe_branch(alternative, source, output)
        });
        Self {
            ordinary_declaration_seen: branch.ordinary_declaration_seen
                && other.ordinary_declaration_seen,
            placement_invalid: branch.placement_invalid
                && other.placement_invalid,
        }
    }
}
...
fn collect_cplusplus_module_declarations(
    ...
    let mut position = CplusplusModulePosition::default();
    let mut cursor = root.walk();
    for child in root.named_children(&mut cursor) {
        position.observe(child, source, output);
    }
}
/// 识别 C++ 模块声明、导入和其他节点
...
fn collect_nested_cplusplus_imports(
    ...
    if node.kind() == "import_declaration" {
        push_cplusplus_module_import(node, source, true, output);
        return;
    }
    let mut cursor = node.walk();
    for child in node.named_children(&mut cursor) {
...
fn push_cplusplus_module_import(
    ...
    push_dependency_declaration(Language::Cplusplus, node, source, 0, output);
    output
        .last_mut()
        .expect("import was just recorded")
        .module_placement_invalid = placement_invalid;
}
...
fn push_dependency_declaration(
    ...
    let point = node.start_position();
    let rust_tree = (language == Language::Rust)
        .then(|| {
            node.child_by_field_name("argument")
                .and_then(|argument| observe_rust_use_tree(argument, source))
        })
        .flatten();
    output.push(DependencyDeclaration {
        ...
        python_modules: if language == Language::Python {
            observe_python_dependency_modules(node, source)
        } else {
            Vec::new()
        },
        rust_tree,
        module_placement_invalid: false,
    });
}

/// 保留多模块导入的每个来源，from 导入保持一个语句级来源
fn observe_python_dependency_modules(
    node: Node<'_>,
    source: &[u8],
) -> Vec<PythonDependencyModule> {
    if matches!(
        node.kind(),
        "import_from_statement" | "future_import_statement"
    ) {
        let point = node.start_position();
        return vec![PythonDependencyModule {
            key: if node.kind() == "future_import_statement" {
                "__future__".to_owned()
            } else {
                dependency_key(Language::Python, node, source)
                    .unwrap_or_default()
            },
            line: point.row + 1,
            column: point.column + 1,
            future_directive: node.kind() == "future_import_statement",
        }];
    }
    let mut cursor = node.walk();
    node.children_by_field_name("name", &mut cursor)
        .map(|name| {
            let module = name.child_by_field_name("name").unwrap_or(name);
            let point = module.start_position();
            PythonDependencyModule {
                key: python_dependency_path(module, source)
                    .unwrap_or_default(),
                line: point.row + 1,
                column: point.column + 1,
                future_directive: false,
            }
        })
        .collect()
}

/// 从直接导入语法取得结构事实，未知节点不猜成普通路径
fn observe_rust_use_tree(
    node: Node<'_>,
    source: &[u8],
) -> Option<RustUseTree> {
    let point = node.start_position();
    let mut tree = RustUseTree {
        path: Vec::new(),
        tail: RustUseTail::Name(String::new()),
        line: point.row + 1,
        column: point.column + 1,
    };
    match node.kind() {
        "use_list" => {
            let mut cursor = node.walk();
            tree.tail = RustUseTail::List(
                node.named_children(&mut cursor)
                    .filter(|child| {
                        !["line_comment", "block_comment"]
                            .contains(&child.kind())
                    })
                    .map(|child| observe_rust_use_tree(child, source))
                    .collect(),
            );
        }
        "scoped_identifier" | "scoped_use_list" => {
            tree.path = match node.child_by_field_name("path") {
                Some(path) => observe_rust_use_tree(path, source)?.path,
                None => vec![String::new()],
            };
            let child = node
                .child_by_field_name("name")
                .or_else(|| node.child_by_field_name("list"))?;
            let child = observe_rust_use_tree(child, source)?;
            tree.path.extend(child.path);
            tree.tail = child.tail;
        }
        "use_as_clause" => {
            tree.path = observe_rust_use_tree(
            ...
            let alias = alias.utf8_text(source).ok()?;
            tree.tail =
                RustUseTail::Name(alias.trim_start_matches("r#").to_owned());
        }
        "use_wildcard" => {
            tree.tail = RustUseTail::Wildcard;
            let mut cursor = node.walk();
            let path = node.named_children(&mut cursor).find(|child| {
                !matches!(child.kind(), "line_comment" | "block_comment")
            });
            if let Some(path) = path {
                tree.path = observe_rust_use_tree(path, source)?.path;
            }
        }
        "identifier" | "self" | "super" | "crate" => {
            let name = node.utf8_text(source).ok()?;
            tree.path.push(name.trim_start_matches("r#").to_owned());
        }
        _ => return None,
    }
    Some(tree)
...
fn dependency_key(
    ...
    if language == Language::Python && node.kind() == "future_import_statement"
    {
        return Some("__future__".to_owned());
    }
    ...
    if language == Language::Python {
        return python_dependency_path(key, source);
    }
...
/// 按标识符和点号重建 Python 来源路径，排除空白与续行
fn python_dependency_path(node: Node<'_>, source: &[u8]) -> Option<String> {
    match node.kind() {
        "identifier" | "." => node.utf8_text(source).ok().map(str::to_owned),
        "dotted_name" | "relative_import" | "import_prefix" => {
            let mut cursor = node.walk();
            node.children(&mut cursor)
                .map(|child| python_dependency_path(child, source))
                .collect::<Option<Vec<_>>>()
                .map(|parts| parts.concat())
        }
        "line_continuation" | "comment" => Some(String::new()),
        _ => None,
    }
}
...
fn blank_lines_between(
    ...
    source.get(end_byte..start_byte).map_or(0, |gap| {
        gap.iter()
            .filter(|byte| **byte == b'\n')
            .count()
            .saturating_sub(1)
...
fn close_complete_source(
    ...
    for callable in &callables {
        ...
        let mut report = |operator, observation: &str| {
            findings.push(
                operator,
                callable.line,
                callable.column,
        ...
        if summary.is_none_or(|line| {
            ...
            report(RuleOperator::DocumentationSummary, &attached);
            continue;
        }
        ...
        if controlled_line_has_terminator(
            ...
            report(RuleOperator::DocumentationTerminator, &attached);
        }
        let forbidden_safety = callable.language == Language::Rust
            && callable.named
            && !callable.requires_safety
            && rust_documentation_heading_indices(&documented_lines)
                .into_iter()
                .any(|index| documented_lines[index].text == "# Safety");
        if forbidden_safety {
            report(
                RuleOperator::DocumentationPublicContract,
                "observed a Safety section on a safe Rust callable",
            );
        } else if has_public_contract
        ...
        {
            report(RuleOperator::DocumentationSafety, &attached);
        }
    }
    ...
    match declaration.role {
        ...
        IdentifierRole::Typedef => {
            declaration.local_form == LocalIdentifierForm::TypeDefinitionSuffix
                ...
                && matches_snake_case(semantic_name, false)
        }
        IdentifierRole::ModuleNamespace
        | IdentifierRole::Tag
        | IdentifierRole::Lifetime
        | IdentifierRole::Label => matches_snake_case(semantic_name, false),
        IdentifierRole::Alias => {
            matches_snake_case(semantic_name, false)
...
/// 按声明要求的大小写检查蛇形名称，空段同时拒绝首尾下划线
fn matches_snake_case(name: &str, uppercase: bool) -> bool {
...
/// 保留 Rust 真实文档标题的原始位置，排除围栏和缩进代码
fn rust_documentation_heading_indices(
    lines: &[DocumentationLine<'_>],
) -> Vec<usize> {
    ...
    for (index, line) in lines.iter().enumerate() {
        if rust_markdown_line_is_indented_code(line.left_padding) {
            continue;
        }
        let marker = line.text.chars().next();
        if let Some(marker @ ('`' | '~')) = marker {
            ...
            if marker_count >= 3 {
                if let Some((open, minimum)) = fence {
                    if marker == open
                        && marker_count >= minimum
                        && line.text[marker_count..].trim().is_empty()
                    {
                        fence = None;
                    }
                } else if marker != '`'
                    || !line.text[marker_count..].contains('`')
                {
        ...
        if fence.is_none() && profile.is_documentation_heading(line.text) {
            headings.push(index);
        }
    }
    headings
...
fn rust_safety_contract_is_complete(lines: &[DocumentationLine<'_>]) -> bool {
    ...
    let mut safety = None;
    let headings = rust_documentation_heading_indices(lines);
    ...
    body.clone().next().is_some()
        && body.all(|line| {
            !rust_markdown_line_is_indented_code(line.left_padding)
...
fn judge_dependencies(
    ...
    for declaration in declarations {
        ...
        {
            findings.dependency(RuleOperator::DependencyWildcard, declaration);
        }
        if declaration.module_placement_invalid
    ...
    let blocked = match law {
        DependencyProfileLaw::Python {
            ...
            for declaration in declarations {
                let mut known_tier = None;
                let mut mixed = false;
                for module in &declaration.python_modules {
                    if let Some(tier) = module.tier(authority) {
                        mixed |= known_tier.is_some_and(|known| known != tier);
                        known_tier = Some(tier);
                    } else {
                        reasons.push(format!(
                            "{} at {}:{}: {}",
                            classification_blocked,
        ...
        DependencyProfileLaw::Rust {
            ...
            let reasons: Vec<_> = declarations
                .iter()
                .filter(|item| {
                    item.rust_tree
                        .as_ref()
                        .is_none_or(|tree| !tree.is_complete())
                })
                .map(|item| {
                    format!(
                        "{} at {}:{}: {}",
                        unknown_use_blocked, item.line, item.column, item.key
                    )
                })
                .collect();
            (!reasons.is_empty()).then(|| reasons.join("; "))
        ...
        | DependencyProfileLaw::Cplusplus {
            ...
            let mut positions: Vec<_> = declarations
                .iter()
            ...
            (!positions.is_empty()).then(|| {
                let positions: Vec<_> = positions
                    ...
                    .collect();
                format!("{} at {}", unavailable_blocked, positions.join(", "))
            })
        }
    };
    if authority.dependency_reorder_safe(law) {
        check_dependency_order(authority, law, declarations, findings);
    }
    blocked.map_or_else(
        || FactFamilyState::Complete(declarations.len() as u32),
...
/// 在同一分组流程中按各语言合同检查顺序和空行
fn check_dependency_order(
    authority: &CompiledAuthority,
    ...
    for declaration in declarations {
        for pair in declaration.python_modules.windows(2) {
            let left = &pair[0];
            let right = &pair[1];
            let (Some(left_tier), Some(right_tier)) =
                (left.tier(authority), right.tier(authority))
            else {
                continue;
            };
                ...
                || (left_tier == right_tier && left.key > right.key)
            {
                findings.push(
                    ...
                    &right.key,
                    &format!(
                        "observed out-of-order Python import member {}",
                        right.key
                    ),
                );
            }
    ...
    for tree in declarations
        .iter()
        .filter_map(|item| item.rust_tree.as_ref())
    {
        check_rust_use_members(tree, findings);
    }
    for pair in declarations.windows(2) {
        ...
        let (out_of_order, blank_lines) = match law {
            DependencyProfileLaw::Python {
                within_tier_blank_lines,
                cross_tier_blank_lines,
                ..
            } => {
                let (Some(left_module), Some(right_module)) =
                    (left.python_modules.last(), right.python_modules.first())
                else {
                    continue;
                };
                let (Some(left_tier), Some(right_tier)) = (
                    left_module.tier(authority),
                    right_module.tier(authority),
                ) else {
                    continue;
                };
                let order = left_tier > right_tier
                    || (left_tier == right_tier
                        && left_module.key > right_module.key);
                let spacing_known =
                    left.python_modules.iter().all(|module| {
                        module.tier(authority) == Some(left_tier)
                    }) && right.python_modules.iter().all(|module| {
                        module.tier(authority) == Some(right_tier)
                    });
                if !spacing_known {
                    if order {
                        findings
                            .dependency(RuleOperator::DependencyOrder, right);
                    }
                    continue;
                }
                let spacing = if left_tier == right_tier {
                    *within_tier_blank_lines
                } else {
                    *cross_tier_blank_lines
                };
            ...
            DependencyProfileLaw::Rust {
                ...
                let (Some(left), Some(right)) =
                    (&left.rust_tree, &right.rust_tree)
                else {
                    continue;
                };
                ...
                (
                    compare_rust_use_trees(left, right).is_gt(),
                    *within_group_blank_lines,
                )
            }
            _ => continue,
        };
        if out_of_order || right.preceding_blank_lines != blank_lines {
            findings.dependency(RuleOperator::DependencyOrder, right);
        }
    }
}

/// 递归检查每层列表的相邻项并定位逆序的后项
fn check_rust_use_members(
    tree: &RustUseTree,
    findings: &mut FindingState<'_>,
) {
    let RustUseTail::List(members) = &tree.tail else {
        return;
    };
    for pair in members.windows(2) {
        ...
        if left.is_complete()
            ...
            && compare_rust_use_trees(left, right).is_gt()
        {
            let key = right.path.join("::");
            findings.push(
                RuleOperator::DependencyOrder,
                right.line,
                right.column,
                &key,
                &format!("observed out-of-order Rust use-list member {key}"),
            );
        }
    }
    for member in members.iter().flatten() {
        check_rust_use_members(member, findings);
    }
}

/// 按路径段、尾部形态、别名和子列表比较 Rust 导入树
fn compare_rust_use_trees(
    left: &RustUseTree,
    right: &RustUseTree,
) -> std::cmp::Ordering {
    for (left, right) in left.path.iter().zip(&right.path) {
        let order = rust_dependency_compare(left, right);
        if !order.is_eq() {
            return order;
        }
    }
    let common = left.path.len().min(right.path.len());
    let rank = |tree: &RustUseTree| {
        if tree.path.len() > common {
            return 1;
        }
        match tree.tail {
            RustUseTail::Name(_) => 0,
            RustUseTail::Wildcard => 2,
            RustUseTail::List(_) => 3,
        }
    };
    ...
    if let (RustUseTail::Name(left), RustUseTail::Name(right)) =
        (&left.tail, &right.tail)
    {
        return version_compare(left, right).then_with(|| left.cmp(right));
    }
    if let (RustUseTail::List(left), RustUseTail::List(right)) =
        (&left.tail, &right.tail)
    {
        for (left, right) in left.iter().zip(right) {
            let left = left
                .as_ref()
                .expect("only complete Rust use trees are compared");
            let right = right
                .as_ref()
                .expect("only complete Rust use trees are compared");
            let order = compare_rust_use_trees(left, right);
            if !order.is_eq() {
                return order;
            }
        }
...
fn rust_dependency_compare(left: &str, right: &str) -> std::cmp::Ordering {
    let rank = |segment: &str| match segment {
        "self" => 0,
        "super" => 1,
        "crate" => 2,
        _ => 3,
...
/// 按字符和数字段比较文本顺序，数字切片不创建临时字符串
fn version_compare(mut left: &str, mut right: &str) -> std::cmp::Ordering {
    while let (Some(first), Some(second)) =
        (left.chars().next(), right.chars().next())
    {
        let (left_end, right_end, order) = if first.is_ascii_digit()
            && second.is_ascii_digit()
        {
            let left_end = left.bytes().take_while(u8::is_ascii_digit).count();
            let right_end =
                right.bytes().take_while(u8::is_ascii_digit).count();
            let left_number = left[..left_end].trim_start_matches('0');
            let right_number = right[..right_end].trim_start_matches('0');
            let order = left_number
                .len()
                .cmp(&right_number.len())
                .then_with(|| left_number.cmp(right_number));
            (left_end, right_end, order)
        } else {
            (first.len_utf8(), second.len_utf8(), first.cmp(&second))
        };
        if !order.is_eq() {
            return order;
        }
