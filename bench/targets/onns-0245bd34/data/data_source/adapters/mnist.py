...
MNIST_PROVENANCE_URL = "http://yann.lecun.com/exdb/mnist/"
MNIST_LICENSE_NAME = "unknown"


class BaseMNISTDataset(SourceImageDataset):
    """
    仅使用显式准备好的本地IDX文件构造MNIST数据源
    """
    ...
    def __init__(
        ...
        super().__init__(
            ...
            provenance_url=MNIST_PROVENANCE_URL,
            license_name=MNIST_LICENSE_NAME,
            source_metadata={
                "source_format": "idx",
            },
        )
    ...
    def _load_raw_dataset(self, dataset_root: str, is_train: bool) -> RawVisionDataset:
        raw_root = Path(dataset_root) / "mnist" / "raw"
        if raw_root.exists():
        ...
        raise FileNotFoundError(
            f"MNIST local raw files are required at {raw_root}; "
            "prepare the dataset explicitly before constructing BaseMNISTDataset."
        )
