...
文档先说明目的和职责，再说明流程、证据与边界。使用短段落和直接表述；表格用于比较，示例用于解释合同。
同一细节由一份文档负责，其他位置通过链接引用。

README 保留定位、核心模型和使用入口。规则、操作、架构或实现发生变化时，更新对应文档；每次发布检查准确性，
不要求每份文档都修改。正文描述当前行为，优先替换过时内容、合并重复解释；版本差异写入发布说明，测量结果保留在验收收据中。
