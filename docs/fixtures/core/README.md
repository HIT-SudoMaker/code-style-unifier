...
| [measure-self-check.ps1](measure-self-check.ps1) | 执行当前候选检查并生成自检暂存收据，供独立校验后晋升 |
...
耗时按新进程启动至退出的墙钟毫秒记录，p95 使用 nearest rank；峰值工作集按操作系统保存的进程生命周期最大字节数记录。
测量保留进程句柄直到读取 `PROCESS_MEMORY_COUNTERS.PeakWorkingSetSize`，查询失败则停止，不用定时采样下界代替峰值。
工作集包括共享和私有驻留页，不等同于私有分配量；[Windows 定义](https://learn.microsoft.com/en-us/windows/win32/psapi/process-memory-usage-information)给出该量的含义。
测量收据的新协议记录峰值方法，旧采样收据不能作为该方法的证据，必须重测。
...
30 次测量的 nearest-rank p95 是排序后的第 29 项，容易受到少数慢样本影响。
比较版本时先核对二进制、测量方法、负载和环境，再查看完整序列与中位数；
需要判断代码回退时，用相同测量器交替运行前后版本，另用同一二进制对照测量器变化。
单次 p95 差值不能独立证明架构成本。复测必须保留原始结果，不能挑选最低一轮替换不利证据。
...
保留的 `.csu/plans/` 历史资料逐文件登记为维护输入并绑定摘要；准备后新增或修改这些资料也会使候选身份失配。
...
语料输出目录必须不存在或为空；测量暂存文件必须不存在，父目录必须已存在且所有祖先均不能是重解析点。
两类暂存收据都必须位于语料之外；仓库内的生成收据只写入 `target`，避免写后改变已验证候选或语料。
以下占位路径需换成同一次运行的实际位置：
...
性能晋升后，运行自检生成器；它核对当前性能候选身份，执行格式、Clippy、测试和两组源码自检，并检查测前测后输入一致：

```powershell
pwsh -NoProfile -File docs/fixtures/core/measure-self-check.ps1 -Workspace target/<本次运行>/corpus -OutputPath target/<本次运行>/self-staging.json
```

`verify-inputs.ps1 -VerifyReleaseEvidenceOnly` 可以只核对规范性能收据与当前功能候选，不要求已经生成自检收据。
