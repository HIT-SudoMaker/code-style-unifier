...
| D | [documentation_regressions](../../../tests/documentation/mod.rs)，按 carriers / subjects / content / fields 分组 |
...
| `aliased_import`／无别名 import | 显式 alias 为 Alias；无独立文档 | 支持 alias 字段；DeclarationReview 无普通 import 名称入口，不把依赖导入路径当自选 alias。| I 前述 import aliases 测试；`tests/dependency/observation.rs — python_dependency_paths_ignore_spacing_and_continuations` 属依赖族证据，不能代替无别名命名计数反例 |
...
| C/C++ type_definition、enumerator | C typedef为Typedef，C++ typedef为Type；enumerator为Enumerator；枚举项有文档，typedef 不在该非 callable 入口 | 支持每个 typedef declarator；C `_t` 有本地形式。`push_native_family_field_declarators` | I `procedural_source_observes_all_direct_declaration_kinds`；I 四语言 alias；F `native_typedef_names_keep_language_specific_forms`保留C/C++同源码对偶；多 typedef 列表组合无专项证据声明 |
...
| 具体义务 | 公共入口证据 |
|---|---|
| Python模块首条真实语句，前导注释不遮蔽模块docstring | D `python_module_documentation_ignores_leading_comment_trivia` |
| Rust公开trait默认实现保留直接trait归属，嵌套函数不提升 | D `rust_public_trait_defaults_keep_direct_owner_visibility` |
| Rust公开tuple位置字段有文档义务，不虚构自选名称 | D `rust_public_tuple_fields_keep_positional_documentation` |
| Rust字面量原生转义/raw语法、多个文档片段按顺序附着 | D `rust_documentation_attributes_decode_native_text_literals`、`rust_documentation_fragments_keep_source_order_and_attachment` |
| Rust普通注释不打断已附属文档，也不能自行充当文档 | D `rust_documentation_trivia_cannot_supply_or_reassign_carriers` |
| Rust安全私有函数也禁止真实Safety部分，代码示例不提供标题 | D `rust_safe_internal_documentation_rejects_real_safety_sections` |
| C/C++ static只来自直接声明，不取函数体局部变量；定义型typedef载体归其直接声明 | C `native_function_visibility_ignores_body_storage_and_modifier_order`、`native_defined_type_documentation_uses_its_declaration_wrapper` |
| Rust匿名导入别名是语法丢弃位置，普通别名仍检查 | I `rust_anonymous_import_aliases_are_structural_discards` |
