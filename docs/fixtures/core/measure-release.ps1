...
# Keep the process handle open through the final OS lifetime-peak query.
# Sampling WorkingSet64 can miss spikes; Process.Refresh loses exited-process data.
if (-not ("Csu.ProcessMemory" -as [type])) {
...
namespace Csu {
    public static class ProcessMemory {
        ...
        private static extern bool GetProcessMemoryInfo(
            SafeProcessHandle process, out Counters counters, uint size);
        public static long ReadPeakWorkingSetBytes(SafeProcessHandle process) {
            Counters counters;
            if (!GetProcessMemoryInfo(process, out counters,
                    (uint)Marshal.SizeOf(typeof(Counters)))) {
                throw new Win32Exception(Marshal.GetLastWin32Error());
            }
            long peak = checked((long)counters.PeakWorkingSetSize.ToUInt64());
            if (peak <= 0) {
                throw new InvalidOperationException("OS memory peak is unavailable");
            }
            return peak;
        }
    }
...
function Invoke-ReviewProcess {
    ...
    # Acquiring SafeHandle before exit retains the process object for PSAPI.
    ...
    try {
        $peakWorkingSetBytes = [Csu.ProcessMemory]::ReadPeakWorkingSetBytes(
            $processHandle
        )
    }
    catch {
...
for ($parent = Get-Item -LiteralPath $outputParent; $null -ne $parent;
    ...
    if ($parent.Attributes.HasFlag([IO.FileAttributes]::ReparsePoint)) {
        throw "Measurement staging output must not traverse a reparse point"
    }
}
...
if ($outputFullPath.StartsWith(
    ...
    ) -and -not $outputFullPath.StartsWith(
        $targetPrefix, [StringComparison]::OrdinalIgnoreCase
    )) {
    throw "Measurement staging inside the repository must stay under target"
}
if (-not (Test-Path -LiteralPath $workspacePath -PathType Container)) {
    throw "Missing corpus workspace: $workspacePath"
}
...
$receipt = [pscustomobject][ordered]@{
    ...
    peak_working_set_method = "win32_process_memory_counters_lifetime_peak_bytes"
