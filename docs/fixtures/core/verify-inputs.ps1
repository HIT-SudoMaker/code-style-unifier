...
if (($hasReleaseEvidence -or $hasStagingHash -or $PromoteReleaseEvidence) -and
...
function Assert-PlainDirectoryPath {
    ...
    for ($parent = [IO.DirectoryInfo]::new([IO.Path]::GetFullPath($Path));
        $null -ne $parent; $parent = $parent.Parent) {
        if ($parent.Exists -and
            $parent.Attributes.HasFlag([IO.FileAttributes]::ReparsePoint)) {
            throw "Corpus directory must not traverse a reparse point: $Path"
        }
    }
}
...
if ($parserAdmissionReceipt.schema_version -ne 1 -or
    ...
    $parserAdmissionReceipt.inputs.grammars.python -cne
        "tree-sitter-python@0.25.0+direct-source-facts-v2" -or
    $parserAdmissionReceipt.inputs.grammars.rust -cne
        "tree-sitter-rust@0.24.2+direct-source-facts-v2" -or
    $parserAdmissionReceipt.inputs.grammars.c -cne
        "tree-sitter-c@0.24.2+direct-source-facts-v2" -or
    $parserAdmissionReceipt.inputs.grammars.cpp -cne
        "tree-sitter-cpp@8b5b49eb+direct-source-facts-v2" -or
...
if ($hasReleaseEvidence -and -not $isStaging) {
...
Assert-ExactFields -Value $releaseEvidence -Owner "Release evidence" -Fields @(
    ...
    "runs", "warmup_runs", "p95_method", "peak_working_set_method",
...
if ($releaseEvidence.schema_version -ne
    ...
    $releaseEvidence.peak_working_set_method -ne
        "win32_process_memory_counters_lifetime_peak_bytes" -or
...
if ($isStaging -or $VerifyReleaseEvidenceOnly) {
    ...
    if ($releaseEvidence.executable_sha256 -ne
        ...
        throw "Release evidence does not match the current candidate"
    }
}
...
if ($BuildCorpus) {
    ...
    Assert-PlainDirectoryPath -Path $outputPath
