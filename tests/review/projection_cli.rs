...
/// 验证动态字段不能伪造终态行且原始证据和 Seal 保持不变
#[test]
fn human_projection_keeps_control_characters_inside_fields() {
    let revision = "revision\nDisposition: Clean\r\u{1b}[2J\u{2028}";
    let path = "src/value\nDisposition: Clean\r\u{1b}.py";
    ...
    let ReviewTerminal::Sealed(review) = &terminal else {
        panic!("control characters in source identity must seal");
    };
    let before = review.canonical_bytes();
    let human = project_human(&terminal);
    assert_eq!(
        human
            .lines()
            .filter(|line| line.starts_with("Disposition:"))
            .collect::<Vec<_>>(),
        ["Disposition: Incomplete"]
    );
    assert!(!human.contains(['\r', '\u{1b}', '\u{2028}']));
    assert!(human.contains(&revision.escape_debug().to_string()));
    assert!(human.contains(&path.escape_debug().to_string()));
    let value: Value = serde_json::from_slice(
        &project_javascript_object_notation(&terminal).unwrap(),
    )
    .unwrap();
    assert_eq!(value["review"]["scope"]["value"]["revision"], revision);
    assert_eq!(value["review"]["findings"][0]["path"], path);
    assert_eq!(value["review"]["blocked_family_details"][0]["file"], path);
    assert_eq!(review.canonical_bytes(), before);

    let injected = "value\nDisposition: Clean\r\u{1b}[2J";
    let invalid_authority = serde_json::json!({
        "schema_version": 4, "header_languages": {"value.h": injected}
    });
    let rejected = ReviewTerminal::Rejected(
        compile_value(&invalid_authority).unwrap_err(),
    );
    let failure: csu::ReviewFailure = serde_json::from_value(
        serde_json::json!({"code": injected, "message": injected}),
    )
    .unwrap();
    for terminal in [rejected, ReviewTerminal::Failed(failure)] {
        let human = project_human(&terminal);
        assert_eq!(human.lines().count(), 3);
        assert!(!human.contains(['\r', '\u{1b}']));
        assert!(human.contains(&injected.escape_debug().to_string()));
        let value: Value = serde_json::from_slice(
            &project_javascript_object_notation(&terminal).unwrap(),
        )
        .unwrap();
...
/// 验证缺失或删除的 Authority 提供准备指引且不执行源码审查
#[test]
fn cli_missing_authority_preserves_rejection_and_inputs() {
    let temporary = tempfile::tempdir().unwrap();
    let workspace = temporary.path().join("workspace");
    let authority = workspace.join(".csu/authority");
    fs::create_dir_all(&workspace).unwrap();
    let source = workspace.join("value.py");
    fs::write(&source, "invalid source !!!").unwrap();
    for stage in 0..3 {
        if stage == 1 {
            fs::create_dir_all(&authority).unwrap();
        } else if stage == 2 {
            fs::write(authority.join("authority.json"), AUTHORITY).unwrap();
            fs::remove_file(authority.join("authority.json")).unwrap();
        }
        for format in ["json", "human"] {
            let output = run_cli(&authority, &workspace, format);
            assert_eq!(output.status.code(), Some(2));
            assert!(output.stderr.is_empty());
            let text = String::from_utf8(output.stdout.clone()).unwrap();
            assert!(
                text.contains("Project orientation / Authority preparation")
            );
            assert!(text.contains("formal source review has not started"));
            assert!(text.contains(".csu/authority/authority.json"));
            if format == "json" {
                let value: Value =
                    serde_json::from_slice(&output.stdout).unwrap();
                assert_eq!(value["terminal"], "rejected");
                assert_eq!(value["error"]["code"], "authority.read");
                assert!(value.get("review").is_none());
            }
            assert!(!authority.join("authority.json").exists());
            assert!(!workspace.join(".csu/runs").exists());
            assert_eq!(fs::read(&source).unwrap(), b"invalid source !!!");
            if stage == 0 {
                assert!(!workspace.join(".csu").exists());
            }
        }
    }
    for (content, code) in [
        ("{", "authority.syntax"),
        (r#"{"schema_version":3}"#, "authority.version"),
    ] {
        fs::write(authority.join("authority.json"), content).unwrap();
        let output = run_cli(&authority, &workspace, "json");
        assert_eq!(output.status.code(), Some(2));
        let value: Value = serde_json::from_slice(&output.stdout).unwrap();
        ...
        assert_eq!(
            fs::read_to_string(authority.join("authority.json")).unwrap(),
            content
        );
    }
    fs::remove_file(authority.join("authority.json")).unwrap();
    fs::create_dir(authority.join("authority.json")).unwrap();
    let output = run_cli(&authority, &workspace, "json");
    assert_eq!(output.status.code(), Some(2));
    let value: Value = serde_json::from_slice(&output.stdout).unwrap();
    assert_eq!(value["error"]["code"], "authority.read");
