...
/// 验证 Rust 匿名导入别名丢弃名称但保留普通别名检查
#[test]
fn rust_anonymous_import_aliases_are_structural_discards() {
    for (source, count) in [
        ("use crate::velocity as _;\n", 1),
        ("use crate::{velocity as _, duration as _};\n", 2),
        ("extern crate velocity as _;\n", 1),
    ] {
        let result = review_source("src/import.rs", source);
        assert_eq!(identifier_count(&result), count, "{source}");
        assert_single_rule(&result, None);
    }
    for source in [
        "use crate::velocity as _value;\n",
        "extern crate velocity as BadName;\n",
    ] {
        assert_single_rule(
            &review_source("src/import.rs", source),
            Some(HARD_CANONICAL),
        );
    }
}
