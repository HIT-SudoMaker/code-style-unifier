...
/// 验证 C 与 C++ typedef 保留各自的类型名称形式
#[test]
fn native_typedef_names_keep_language_specific_forms() {
    for (path, source, valid) in [
        ("src/type.c", "typedef int velocity_t;\n", true),
        ("src/type.c", "typedef int Velocity;\n", false),
        ("src/type.cpp", "typedef int Velocity;\n", true),
        ("src/type.cpp", "typedef int velocity_value;\n", false),
    ] {
        let result = review(&[(path, source)]);
        let findings: Vec<_> = result
            ...
            .collect();
        if valid {
            assert!(findings.is_empty(), "{path}: {source}: {findings:?}");
        } else {
            assert_eq!(findings.len(), 1, "{path}: {source}");
            assert_eq!(findings[0].rule(), "identifier.canonical_form");
        }
    }
}
