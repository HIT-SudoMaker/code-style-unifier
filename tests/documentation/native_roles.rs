...
fn reviewer_for_path(
    ...
    .unwrap();
    if !public_callables.is_empty() {
        authority["public_callables"][path] =
            serde_json::json!(public_callables);
    }
...
/// 验证原生内部归属只读取函数自身的存储说明符
#[test]
fn native_function_visibility_ignores_body_storage_and_modifier_order() {
    let mut authority: serde_json::Value = serde_json::from_str(include_str!(
        "../../docs/fixtures/core/authority.json"
    ))
    .unwrap();
    ...
    authority["token_vocabulary"] =
        serde_json::json!(["calculate", "velocity", "value"]);
    let reviewer = compile_value(&authority).unwrap();
    for (path, parameters) in [("src/value.c", "void"), ("src/value.cpp", "")]
    {
        for (prefix, body, complete) in [
            ("", "int value;", false),
            ...
            let source = format!(
                "/**\n * 计算数值\n */\n{prefix}void calculate_velocity({parameters}) {{ {body} }}\n"
            );
        ...
        let source = format!(
            "/**\n * 计算数值\n */\nstatic void calculate_velocity({parameters}) {{}}\n"
        );
        let result = review_sources(
            &reviewer_for_path(&["calculate_velocity"], path),
            "native-authority-tier",
            &[(path, &source)],
        );
        assert!(has_public_contract_finding(&result));
    }
}

/// 验证 typedef 结构定义采用直接所属声明的紧邻文档
#[test]
fn native_defined_type_documentation_uses_its_declaration_wrapper() {
    for (path, tag) in [("src/value.c", "sample"), ("src/value.cpp", "Sample")]
    {
        let alias = if path.ends_with(".c") {
            "sample_t"
        } else {
            "SampleType"
        };
        ...
        let ReviewTerminal::Sealed(result) = review_sources(
            &reviewer_for_path(&[], path),
            "type-wrapper",
            &[(path, &source)],
        ) else {
            panic!("type source must seal")
        };
        ...
        let findings: Vec<_> = result
            ...
            .collect();
        assert_eq!(findings.len(), 1, "{detached}");
    }
}
